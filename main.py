...
def init_weights(m):
    if type(m) not in [nn.ReLU, nn.LocalResponseNorm, nn.MaxPool2d, nn.Sequential, nn.Dropout, ZFNet]:
        torch.nn.init.normal_(m.weight, 0, 0.01)
        m.bias.data.fill_(1)

zfnet = ZFNet(num_classes=1000)
zfnet.apply(init_weights)
zfnet = zfnet.to(device)

criterion = nn.CrossEntropyLoss().cuda()
optimizer = optim.SGD(zfnet.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0005)
